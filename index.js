...
client.on('interactionCreate', async interaction => {
    ...
    if(funny === 'first') {
        ...
        .setDescription("● To get help on a specific command type `@Rythm bot Music help <command>`!")
        .setColor("#303037")
        .setFooter("Rythm bot Music", ee.avatara)
        .setTimestamp()
...
client.on('interactionCreate', async interaction => {
    ...
    if(player === undefined) {
        if(interaction.channel.name === 'Rythm-requets') {
            setTimeout(() => {
...
client.on('guildCreate', async (guild) => {
    ...
    client.channels.cache.get("881204772160561165").send({embeds: [embed]})
  ...
  try {
    ...
    const texts = "Hi, This Is Rythm If You Need Support Related Me Join My Support Server. Support Server Link Is Here https://discord.gg/u9x5gdZ6Vw"
